...
def on_startup():
    ...
    threading.Thread(
        target=subscribe_to_pubsub, daemon=True
    ).start()  # Start the subscriber in a daemon thread
    create_database()
...
@app.get("/failure")
def failure(
    request: Request, reason: str = Query(None, description="Reason for failure")
):
    if not reason:
        reason = "Unknown error"
    return templates.TemplateResponse(
        "failure.html", {"request": request, "reason": reason}
    )
